...
//     async facultytimetableMenu(){
//       "https://jubilant-darkness-qidltchfum5o.on-vapor.com/admin/lms?comp=faculty_timetable",{
...
class FacultyTimeTable {
    ...
    async fillFacultyTimeTable(data) {
        ...
        // Programme name - Using Choices JS
        await this.programmeNameDropdown.click();
        await this.searchProgrammeOption.fill(String(data.programmeName));
        await this.searchProgrammeOption.press("Enter");
        

        // Wait until course dropdown loads actual options
        await this.page.waitForFunction(() => {
            const el = document.querySelector("#courses_list");
            return el && el.options.length > 1;
        }, { timeout: 10000 });

        // Select course dynamically from Excel
        await this.courseDropdown.click();
        await this.courseDropdown.fill(String(data.Course));
    ...
    async showclick() {
        await this.showTimeTableBtn.click();
        console.log("✔ Faculty Time Table form submitted");
    }
}
